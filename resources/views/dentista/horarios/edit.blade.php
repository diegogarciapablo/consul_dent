        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('dentista.horarios.update', $horario) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <select id="dia_semana" name="dia_semana" class="mt-1 block w-full rounded-md border-gray-300 bg-white py-2 px-3 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm">
                                ...
                                @foreach ($dias as $index => $dia)
                                    <option value="{{ $index }}" {{ old('dia_semana', $horario->dia_semana) == $index ? 'selected' : '' }}>{{ $dia }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input id="hora_inicio" name="hora_inicio" type="time" value="{{ old('hora_inicio', substr($horario->hora_inicio, 0, 5)) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        ...
                        <div>
                            ...
                            <input id="hora_fin" name="hora_fin" type="time" value="{{ old('hora_fin', substr($horario->hora_fin, 0, 5)) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
