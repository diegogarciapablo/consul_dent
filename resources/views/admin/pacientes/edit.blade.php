                ...
                <div class="p-6 text-gray-900">
                    <form action="{{ route('admin.pacientes.update', $paciente) }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <select id="user_id" name="user_id" class="mt-1 block w-full rounded-md border-gray-300 bg-white py-2 px-3 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm">
                                ...
                                @foreach ($usuarios as $usuario)
                                    <option value="{{ $usuario->id }}" {{ old('user_id', $paciente->user_id) == $usuario->id ? 'selected' : '' }}>{{ $usuario->name }} - {{ $usuario->email }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input id="fecha_nacimiento" name="fecha_nacimiento" type="date" value="{{ old('fecha_nacimiento', optional($paciente->fecha_nacimiento)->format('Y-m-d')) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        ...
                        <div>
                            ...
                            <input id="telefono" name="telefono" type="text" value="{{ old('telefono', $paciente->telefono) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        ...
                        <div>
                            ...
                            <textarea id="direccion" name="direccion" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">{{ old('direccion', $paciente->direccion) }}</textarea>
